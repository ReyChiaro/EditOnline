...
class QwenImageMaskEditPipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    r"""
    ...
    def prepare_masks(
        self,
        mask_images: torch.Tensor | list[torch.Tensor],
        batch_size: int,
        height: int,
        width: int,
        dtype: torch.dtype,
        ...
        if mask_images == [] or mask_images is None:
            return None

        height = int(height) // (self.vae_scale_factor * 2)
        width = int(width) // (self.vae_scale_factor * 2)

        if not isinstance(mask_images, list):
            mask_images = [mask_images]
        
        packed_masks = []
        for mask in mask_images:
            mask: torch.Tensor = F.interpolate(mask, (1, height, width), mode="nearest")
            mask = mask.mean(dim=1, keepdim=True)
            mask_latents = mask.view(batch_size, 1, height, width)
            mask_latents = mask_latents.flatten(-2)
            mask_latents = mask_latents.permute(0, 2, 1)
            if num_channels_latents is not None:
                mask_latents = mask_latents.repeat_interleave(num_channels_latents, dim=-1)
            packed_masks.append(mask_latents)
        packed_masks = torch.cat(packed_masks, dim=1)
        packed_masks = packed_masks.to(device=device, dtype=dtype)
        return packed_masks
    ...
    def __call__(
        ...
        mask: PipelineImageInput | None = None,
        prompt: str | list[str] = None,
        ...
        unmask_with: Literal["target", "source", "noisy_source", "noisy_target"] = "target",
        enable_concat_mask: bool = False,
    ):
        r"""
        ...
        if image is not None and not (isinstance(image, torch.Tensor) and image.size(1) == self.latent_channels):
            ...
            for i, img in enumerate(image):
                ...
                if mask is not None:
                    vae_masks.append(
                        T.to_tensor(self.image_processor.resize(mask[i], vae_height, vae_width))
                        .unsqueeze(0)
                        .unsqueeze(2)
                    )
        ...
        mask_latents = None
        if vae_masks:
            mask_latents = self.prepare_masks(
                mask_images=vae_masks,
            ...
            if unmask_with in ["target", "source"]:
                latents = mask_latents * latents + (1 - mask_latents) * image_latents
            elif unmask_with in ["noisy_target", "noisy_source"]:
                latents = latents
            else:
                latents = latents

        if not enable_concat_mask:
            img_shapes = [
                [
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if enable_concat_mask and mask_latents is not None:
                    latent_model_input = torch.cat([latent_model_input, mask_latents], dim=1)
                ...
                # Add mask after every denoise step
                    ...
                    if unmask_with in ["source", "target"]:
                        ...
                        if i < len(timesteps) - 1:
                            ...
                            noisy_image_latents = self.scheduler.scale_noise(
                                noisy_image_latents, torch.tensor([noise_timestep]), noise
                            )
                        latents = (1 - mask_latents) * noisy_image_latents + mask_latents * latents
